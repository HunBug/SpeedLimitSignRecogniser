...
public:
	...
	//singleton template lekérés

	TemplateMatchingDetector();
	virtual ~TemplateMatchingDetector();

	virtual std::vector<cv::Rect2i> getSigns(cv::Mat source, cv::Mat candidateMap) override;
private:
	bool tryNextCandidate(cv::Mat source, cv::Mat& candidateMap, cv::Rect2i& sign);
};
